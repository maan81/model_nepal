      ...
      <?php if($contests_slideshow): ?>
         <div class="upcomming">
            <div class="upcomming_selectors">
               <?php foreach($contests_slideshow as $key=>$val):?>
                  <div class="upcomming_selector"></div>
               <?php endforeach?>
            </div>
            <?php foreach($contests_slideshow as $key=>$val): ?>
               <div class="eventpic">
                  <a href="<?php echo site_url('contests/'.$val->link)?>">
                     <span class="title"><?php echo $val->title?></span>
                     <img  title="<?php echo $val->title?>" 
                           alt="<?php echo $val->title?>" 
                           width="674" height="315"
                           src="<?php echo base_url().CONTESTSPATH.gen_folder_name($val->title)?>.jpg" />
                  </a>
               </div>
            <?php endforeach?>
         </div>
               ...
               <tr style="text-align:center">
                  <td>
                     <select class="modelparam" name="title" style="width:140px;">
                        <option selected="selected" value="">Contest Name</option>
                        <?php 
                           $titles=array();
